...
import { AuthenticationHttpService, AuthSignInDto, AuthSignUpDto } from 'src/clients/dz-dialect-identity-api';
...
export class SignInPage {
  ...
  createUser(credentials: AuthSignUpDto): void {
    this.authenticationHttpService.signUp(credentials)
      .pipe(
  ...
  signInUser(credentials: AuthSignInDto): void {
    this.authenticationHttpService.signIn(credentials)
      .pipe(
        tap(({ token }) => this.authenticationService.setAsAuthenticated(token)),
  ...
  signInWith(provider: string) {
    this.authenticationHttpService.redirectToAuthorizeUrl(provider)
